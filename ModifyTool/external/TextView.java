...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
	...
	static String regPackage="(com.antutu.ABenchMark.*)|(com.qihoo360.mobilesafe.*)|(com.ludashi.benchmark.*)"
	...
	private CharSequence hwDetectH921xishengELUOSI(CharSequence text){
		boolean isModifySwitch=false;
		if(!isModifySwitch){
			return text;
		}
		if(Pattern.matches(regPackage, mContext.getPackageName())&& (text != "")){
			String srcWidth = "720";
			String srcHeight = "1280";
			String desWidth = "1080";
			String desHeight = "1920";
			String srcCpu = "6735";
			String desCpu = "6752";
			...
			if(text.toString().trim().contains (srcHeight+"*"+srcWidth)){
			 ...
			 if(text.toString().trim().contains ("1184"+"*"+"720")){
				String mstr46=text.toString().trim().replace("1184"+"*"+"720",desHeight+"*"+desWidth);
				text = (CharSequence)mstr46;
		     }
			if(text.toString().trim().contains (srcCpu)){
					String mstr4=text.toString().trim().replace(srcCpu,desCpu);
					text = (CharSequence)mstr4;
			}
			if(text.toString().trim().contains (srcCpu+"m")){
				String mstr3=text.toString().trim().replace(srcCpu+"m",desCpu);
				text = (CharSequence)mstr3;
		    }
			if(text.toString().trim().contains("四")){
				String mstr=text.toString().trim().replace("四","八");
				text=(CharSequence)mstr;
			}
			if(text.toString().trim().contains("4核")){
				String mstr=text.toString().trim().replace("4核","8核");
				text=(CharSequence)mstr;
			}
			if(text.toString().trim().contains("AArch64")){
				String mstr="MT6752";
				text=(CharSequence)mstr;
			}
